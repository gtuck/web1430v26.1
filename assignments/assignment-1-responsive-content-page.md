...
| **Accessibility** | `lang`, `alt`, contrast, label/id pairs, and heading hierarchy all correct | Four of five criteria met | Two or three criteria met | One criterion met; significant accessibility issues |
| **Constraints and validation** | HTML validates at validator.w3.org with zero errors; no JavaScript, no CSS framework, no `!important`, no inline `style`, no table-based layout | Validates with 1–2 minor warnings; all constraints respected | Several validation errors, or one constraint broken | Not validated, or multiple constraints broken |
