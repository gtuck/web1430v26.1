...
- All 14 labs and the two Week 00 orientation items are generated as real Canvas assignments by the build (July 2026): labs live in the Labs assignment group with points equal to 4 × their rubric criteria and URL+text submission; the orientation items fill the Orientation group. Each is placed in its module directly after the corresponding handout page. The Week 00 "Canvas Orientation Quiz" is authored as `quizzes/quiz-0-canvas-orientation.json` (August 2026): its manifest resources are generated by the build via the `GENERATED_ASSESSMENTS` mapping (stable identifiers), it sits in the Orientation assignment group, and its module item follows the GitHub Repo Setup assignment in Week 00.
...
- Brand-new assessments (no hand-authored manifest resource) must be registered in the build script's `GENERATED_ASSESSMENTS` mapping (title → group, position, module placement); the build then generates their manifest resources with stable identifiers. The Canvas Orientation Quiz is the first assessment added this way.
